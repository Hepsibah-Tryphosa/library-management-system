...
import { EntityArrayResponseType, CollegeInfoService } from '../service/college-info.service';
...
export class CollegeInfoComponent implements OnInit {
  ...
  trackId = (_index: number, item: ICollegeInfo): number => this.collegeInfoService.getCollegeInfoIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.collegeInfos = this.refineData(dataFromBody);
  }

  protected refineData(data: ICollegeInfo[]): ICollegeInfo[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: ICollegeInfo[] | null): ICollegeInfo[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.collegeInfoService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
